...
function init_fancycolors_range(_fancycolors) {
  ...
  jQuery(".brand_switcher").on("click", function(){
		jQuery(".shapediamonds").toggleClass("hidden");
		jQuery(".specialdiamonds").toggleClass("hidden");	
		
		if(jQuery('.shapediamonds').hasClass("hidden")) {jQuery(".brand_switcher").html("Search By Shape"); jQuery("#shapes").val('All');};	
		if(jQuery('.specialdiamonds').hasClass("hidden")){jQuery(".brand_switcher").html("Search By Brand"); jQuery("#specialshapes").val('');  };	
		dtl.re_fetch_result();
  });
...
function init_custom_image_filter(_customimage_array)
{
	if(_customimage_array)
	{
		var _customimage_array = _customimage_array.split(",");
		for (i = 0; i < _customimage_array.length; i ++) 
		{
			jQuery('input[name=custom_image][value="' + _customimage_array[i] + '"]').parent().find('i').removeClass('icons-checkbox').addClass('icons-checked');
			jQuery('input[name=custom_image][value="' + _customimage_array[i] + '"]').parent().attr('data-action', 'Unselect');
...
function init_custom_certificate(_customcert_array)
{
	if(_customcert_array)
	{
		var _customcert_array = _customcert_array.split(",");
		for (i = 0; i < _customcert_array.length; i ++) {
		jQuery('input[name=custom_report][value="' + _customcert_array[i] + '"]').parent().find('i').removeClass('icons-checkbox').addClass('icons-checked');
		jQuery('input[name=custom_report][value="' + _customcert_array[i] + '"]').parent().attr('data-action', 'Unselect');
...
function init_inhouse_products(_inhouse_products)
{
	if(_inhouse_products)
	{
		jQuery('input[name=inhouse_products][value="' + _inhouse_products + '"]').parent().find('i').removeClass('icons-checkbox').addClass('icons-checked');
		jQuery('input[name=inhouse_products][value="' + _inhouse_products + '"]').parent().attr('data-action', 'Unselect');
		show_adv_bar();
	}
}
...
function init_search_panel(_shapes, _cuts, _colors, _fancycolors, _clars, _fluors, _minCarat, _maxCarat, _minPrice, _maxPrice,
  _minRatio, _maxRatio, _collections, _origins, _reports, _stock_number,_special,_custom_imagesarray,_custom_certificatearray,_inhouse_products) {
  init_shapes(_shapes);
  ...
  init_custom_image_filter(_custom_imagesarray);
  init_custom_certificate(_custom_certificatearray);
  //init__custom_certificatearray(_custom_certificatearray);
  init_inhouse_products(_inhouse_products);
}
