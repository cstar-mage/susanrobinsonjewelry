...
function getResultsTable() {
	...
	lastAjax = jQuery.ajax({
		...
		success: function(data){
			//saving data for Compare Checkboxes
			gridCurrentData = data.rows;
...
function fillTable(rows, tablename){
	...
	for(var i=0;i<rows.length;i++){
		...
		var isIn = false;
		for(var j=0;j<compare_params.length;j++){
			if(compare_params[j]==rows[i].id) {isIn=true; break;}
		}
		
		results_html += "<div class='diam-compare'>";
		results_html += "<input type='checkbox' title='Add to Compare' class='chk_compare' data-id='"+rows[i].id+"' "+(isIn?'checked':'')+" />";
		results_html += "</div>";
...
function showDetailsBlock(row,detailsId){
	...
	htm += '<a href="'+row["view"]+'" class="btn btn-lg btn-block btn-viewdiamond" >'+view_button_text+'</a>';
	if(dont_see_what_you_want_button){
		htm += '<a href="'+dont_see_what_you_want_button_link+'" class="btn btn-lg btn-block mt5 btn-dont-see-what-want" ">DON\'T SEE WHAT YOU WANT ?'+'</a>';
	}
	htm += '<div class="row">';
	if(row["image"] && !row["is_samp"])
		htm += '<div class="col-md-12 text-center pt10"><img src="'+row["image"]+'" class="img-responsive"/></div>';
	htm += '<div class="col-md-12 text-center text-uppercase pt10" style="font-weight:bold">'+row["shape_fullname"]+'</div>';
	htm += "</div>";
...
function setShapeRanges(){
	...
	try{
		if(use_rapnet_instant_api_data){
			p_min.push(min_totalprice);
			p_max.push(max_totalprice);
			c_min.push(min_carat);
			c_max.push(max_carat);
		}else{
			jQuery("ul.product-shape li.active").each(function(){
				//try {
...
jQuery(document).ready(function() {
	...
	jQuery(document).on('change','.diam-compare .chk_compare', function(e){
		if(jQuery(this).is(':checked')){
			if(addCompareParamsCookie(jQuery(this).data("id")))
				document.location.href=document.location.href;
			else
				jQuery(this).prop('checked', false);
		}
		else{
			removeCompareParamsCookie(jQuery(this).data("id"),false);
			document.location.href=document.location.href;
		}
