...
class Jewelerslink_Jewelryshare_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getImportCSV() {
		try
		{
			...
			$ch = curl_init();
			$timeout = 5;
			curl_setopt($ch,CURLOPT_URL,"http://www.jewelerslink.com/jewelry/index/getjsonnew");
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, array("username"=>$username,"password"=>$password,"attributes"=>$mapped_string));
			$data = curl_exec($ch);
			curl_close($ch);
			//echo $data; exit;
			...
			if($data == "Invalid Login") {
				...
				foreach($jsonData as $csvRow) {
					if($headerFlag){$csvData[] = $csvRow; $headerFlag=false; continue;}
	...
	public function getUpdateCSV() {
		try
		{
			...
			*/
			...
			$ch = curl_init();
			$timeout = 5;
			curl_setopt($ch,CURLOPT_URL,"http://www.jewelerslink.com/jewelry/index/getMappedUpdateJsonNew");
			curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
			curl_setopt($ch,CURLOPT_CONNECTTIMEOUT,$timeout);
			curl_setopt($ch, CURLOPT_POSTFIELDS, array("username"=>$username,"password"=>$password,"attributes"=>$mapped_string));
			$data = curl_exec($ch);
