<?php
class Mage_Stud_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function addtocartAction()
    {
    	...
    	if($_REQUEST['metalstyle'] == "white_gold"){
    		$_REQUEST['metalstyle'] = "14k White Gold";
    	}else if($_REQUEST['metalstyle'] == "yellow_gold"){
    		$_REQUEST['metalstyle'] = "14k Yellow Gold";
    	}else if($_REQUEST['metalstyle'] == "rose_gold"){
    	...
    	}else if($_REQUEST['metalstyle'] == "18k_rose_gold"){
    		$_REQUEST['metalstyle'] = "18k Rose Gold";
    	}else{
	...
	public function priceupdateAction(){
		...
		if($data['shape'] != "" && $data['carat'] != "" && $data['clarity'] != "" &&  $data['color'] != "")
		{
			...
			if($type == "fgvs")
			{
				...
				$price = $collectiondata['0']['fgvs'];
			}
			else if($type == "fgsi")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['fgsi'];
			}
			else if($type == "fgind")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['fgind'];
			}
			else if($type == "hivs")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['hivs'];
			}
			else if($type == "hisi")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['hisi'];
			}
			else if($type == "hiind")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['hiind'];
			}
			else if($type == "jkvs")
			{
				//$carat1 = floatval($carat);
				$price = $collectiondata['0']['jkvs'];
			}
			else if($type == "jksi")
			{
				$price = $shape_result['jksi'];
			}
			else if($type == "jkind")
			{
				$price = $collectiondata['0']['jkind'];
			}
			else{
				$price = '';
			}
		...
		if($data['shape'] != "" && $data['carat'] != "" && $data['clarity'] != "" &&  $data['color'] != "" && $data['metal_type'] != "")
		{	
			$metal_price = Mage::getStoreConfig('stud/general_settings/'.$data['metal_type']);
