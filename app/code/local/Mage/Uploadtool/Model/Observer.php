...
class Mage_Uploadtool_Model_Observer {
	...
	public function sendReportDiamonds() {
		...
		$daily_jpg=Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'images/daily.jpg';
		$elogo_jpg=Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'images/e_logo.jpg';
		$flogo_jpg=Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'images/f_logo.jpg';
		$fromemailaddr=Mage::getStoreConfig('trans_email/ident_general/email');
		$back_img=Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'images/shadow.png';
		...
		try
		{
			...
			$query = "SELECT $uploadtool_diamonds_inventory.owner as vendor, count($uploadtool_diamonds_inventory.owner) as diams FROM
			`$uploadtool_diamonds_inventory`
			group by $uploadtool_diamonds_inventory.owner";
		...
		font-size: 18px;  margin:10px auto;padding: 8px 0; text-align: center;text-transform: uppercase; width: 91%;'>Diamond Synchronization Report</p></td>
				...
				$messageBodyDiam .= "<table width = '92%' cellspacing='0' style='margin:0 auto'>";
				$messageBodyDiam .= "<thead style='background-color: rgb(102, 102, 102); color: rgb(255, 255, 255); line-height: 31px;'>";
				$messageBodyDiam .= "<th align='left' style='font-family:sans-serif;'>&nbsp;Vendor</th>";
				$messageBodyDiam .= "<th align='right' style='font-family:sans-serif;'>Diamonds&nbsp;</th>";
				$messageBodyDiam .= "</thead>";
				$messageBodyDiam .= "<tbody>";
				while($row = mysql_fetch_array($result))
				{
					$messageBodyDiam .= "<tr>";
					$messageBodyDiam .= "<td align='left'  style='font-family:sans-serif;'>&nbsp;".$row['vendor']."</td>";
					$messageBodyDiam .= "<td align='right' style='font-family:sans-serif;'>".$row['diams']."&nbsp;</td>";
					$messageBodyDiam .= "</tr>";
				}
				$messageBodyDiam .= "</tbody>";
				$messageBodyDiam .= "</table>";
				$emailbool=true;
				$diamond_avail=true;
			}
			else { // $messageBodyDiam .= "No diamonds found";$emailbool="";
			}
		...
		try
		{
			...
			$vendors = $attributeDetails->getSource()->getAllOptions(false);
			...
			if(count($vendors) > 0)
			{
				...
				$messageBodyJewel .= "<table width = '92%' cellspacing='0' style='margin:0 auto'>";
				$messageBodyJewel .= "<thead style='background-color: rgb(102, 102, 102); color: rgb(255, 255, 255); line-height: 31px;'>";
				$messageBodyJewel .= "<th align='left' style='font-family:sans-serif;'>&nbsp;Vendor</th>";
				$messageBodyJewel .= "<th style='font-family:sans-serif;'>Updated Products</th>";
				$messageBodyJewel .= "<th align='right' style='font-family:sans-serif;'>Jewelry&nbsp;</th>";
				$messageBodyJewel .= "</thead>";
				
				foreach($vendors as $productlist)
				{
					$option_id = $productlist["value"];
					
					if($option_id != '' && $option_id != NULL)
					{
						//echo $productlist["label"]." - ";
						$productColl = Mage::getResourceModel('catalog/product_collection');
						$productColl->addAttributeToSelect('*');
						$productColl->addAttributeToFilter('manufacturer', array('eq'=>$option_id));	
						
						//echo $productColl->getSelect()." - ";
						$product_count=$productColl->getSize();
						
						$productColl->addAttributeToSort('updated_at');
						$firstItem = $productColl->getLastItem();
						
						$product_count_date = 0;
						
						if($firstItem->getUpdatedAt())
						{
							$lastupdate=date("Y-m-d 00:00:00",strtotime($firstItem->getUpdatedAt()));
							$lastupdateDisplay = date("m-d-Y",strtotime($firstItem->getUpdatedAt()));
							$dates=date("Y-m-d",strtotime($firstItem->getUpdatedAt()));
							
							$collection = Mage::getModel('catalog/product')->getCollection();
							$collection->addAttributeToSelect('*');
							$collection->addAttributeToFilter('manufacturer', array('eq'=>$option_id));
							$collection->addFieldToFilter('updated_at', array('gteq' => $lastupdate));
							
							$product_count_date=$collection->getSize();
						}
						
						if($product_count > 0)
						{
							$messageBodyJewel .= "<tr>";
							if($product_count_date)
							{
								$messageBodyJewel .= "<td align='left' style='font-family:sans-serif;' >&nbsp;".$productlist["label"]."<br/>Last Updated: ".$lastupdateDisplay."</td>";
							}
							else
							{
								$messageBodyJewel .= "<td align='left' style='font-family:sans-serif;'>&nbsp;".$productlist["label"]."</td>";
							}
							$messageBodyJewel .= "<td align='center' style='font-family:sans-serif;'>".$product_count_date."</td>";
							$messageBodyJewel .= "<td align='right' style='font-family:sans-serif;'>".$product_count."&nbsp;</td>";
							$messageBodyJewel .= "</tr>";
							$emailbool=true;
							$jewelry_avail=true;
						}
				...
				$messageBodyJewel .= "</table>";
				$messageBodyJewel .= "</td></tr>";
			}
			else 
			{ 
			  //$messageBodyJewel .= "No diamonds found";$emailbool="";
			}
			//--- Jewelries Report ------
			$fromuser="support@idealbrandmarketing.com";

			$query1 = "SELECT * from watches_vendor";
			//echo $query;
			...
			if(count($vendorsw) > 0)
			{
				...
				$messageBodyWatch .= "<table width = '92%' cellspacing='0' style='margin:0 auto'>";
				$messageBodyWatch .= "<thead style='background-color: rgb(102, 102, 102); color: rgb(255, 255, 255); line-height: 31px;'>";
				$messageBodyWatch .= "<th align='left' style='font-family:sans-serif;'>&nbsp;Vendor</th>";
				$messageBodyWatch .= "<th align='right' style='font-family:sans-serif;'>Watches</th>";
				$messageBodyWatch .= "</thead>";
				$messageBodyWatch .= "<tbody>";
				foreach($vendorsw as $productlistw)
				{
					
					$attributeDetails = Mage::getSingleton('eav/config')->getAttribute('catalog_product','manufacturer');
					$options = $attributeDetails->getSource()->getAllOptions(false);
					//echo "<pre>";print_r($options).'<br>';
					$option_id = '';
					foreach ($options as $option)
					{
						//$optionLabel = str_replace('-', ' ', $option['label']);
						$optionLabel = $option['label'];
						if (strtolower($optionLabel) == strtolower($productlistw))
						{
					...
					if($option_id != '' && $option_id != NULL)
					{
						...
						{
							$messageBodyWatch .= "<tr>";
							$messageBodyWatch .= "<td align='left' style='font-family:sans-serif;' >".$productlistw."</td>";
							$messageBodyWatch .= "<td align='right' style='font-family:sans-serif;'>".$product_count."&nbsp;</td>";
							$messageBodyWatch .= "</tr>";
							$emailbool=true;
							$watch_avail=true;
						}
		...
		</tr><tr><td><img src='".$daily_jpg."' style='width:100%' /><td></tr>";
		
			if($diamond_avail)$messageBody .= $messageBodyDiam;
			if($jewelry_avail)$messageBody .= $messageBodyJewel;
			if($watch_avail)$messageBody .= $messageBodyWatch;
			...
			if($emailbool){
				...
				$subject = "JewelersLink Daily Report - ".date("m/d/Y");
				
				mail($jewelryto,$subject,$messageBody,$headers);
				echo 'Report sent';
			}else{
				echo 'No data to be reported';
			}
			return $this;
		}
		catch (Exception $e) { 
