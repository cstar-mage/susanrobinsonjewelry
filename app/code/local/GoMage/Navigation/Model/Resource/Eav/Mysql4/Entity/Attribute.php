	        ...
	        if (is_array($option)) {
	            ...
	            if (isset($option['value'])) {
	                ...
	                foreach ($option['value'] as $optionId => $values) {
						...
						if(isset($_FILES['option']['name']['image'][$optionId]) && $_FILES['option']['name']['image'][$optionId] != "") {
						
							//echo $_FILES['option']['name']['image'][$optionId]; exit;
							try {
								
								/* Starting upload */
								//$uploader = new Varien_File_Uploader('option');
								$uploader = new Varien_File_Uploader(
										array(
												'name' => $_FILES['option']['name']['image'][$optionId],
												'type' => $_FILES['option']['type']['image'][$optionId],
												'tmp_name' => $_FILES['option']['tmp_name']['image'][$optionId],
												'error' => $_FILES['option']['error']['image'][$optionId],
												'size' => $_FILES['option']['size']['image'][$optionId]
										)
								);
								...
								$path = Mage::getBaseDir('media').DS.'option_image'. DS;
								//echo $path; exit;
								
								$uploader->save($path, $_FILES['option']['name']['image'][$optionId]);
									
							} catch (Exception $e) {
								//echo $e->getMessage(); exit;
							}
							$result = array();
							$result[] = array('file'=>$_FILES['option']['name']['image'][$optionId],'name'=>$_FILES['option']['name']['image'][$optionId],'size'=>$_FILES['option']['size']['image'][$optionId],'status'=>'new');
							$option['image'][$optionId] = Mage::helper('core')->jsonEncode($result);
						}
