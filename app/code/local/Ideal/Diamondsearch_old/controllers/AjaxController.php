...
class Ideal_Diamondsearch_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function listAction()
    {
    	...
    	//echo "shape in ($shape) and color in ($color)";
    	//echo '((`shape` in ('.$shape.')) and (`color` in ('.$color.')) and (`clarity` in ('.$clariti.')) and (`certificate` in ('.$certificate.'))) ';
    	...
    	if($_item["text"])
    	{
    		$specialshapesLabelArray[$_item["label"]]=$_item["text"];
    	}
    	endforeach;
		...
		if(isset($_REQUEST["special"]) && $_REQUEST["special"] != '' )
		{
			$spacialSearchPara=array();
			$searchlabel=array();
			$searchlabel=explode(",", $_REQUEST["special"]);
			...
			$diamondsCollection->addFieldToFilter('trim(special_shape)', array('in' => $spacialSearchPara));
		}
		...
		*/
		if(isset($_REQUEST["cuts"]) && count($cutsArray) != count(explode(",", $_REQUEST["cuts"])))
		{
			$diamondsCollection->addFieldToFilter('cut', array('in' => Mage::helper("diamondsearch")->getShortCodes("cut", explode(",", $_REQUEST["cuts"])) ));
		}
		/*
		...
		*/
		if(isset($_REQUEST["fluorescences"]) && count($fluorescencesArray) != count(explode(",", $_REQUEST["fluorescences"])))
		{
			$diamondsCollection->addFieldToFilter('fluorescence', array('in' => Mage::helper("diamondsearch")->getShortCodes("fluorescence", explode(",", $_REQUEST["fluorescences"])) ));
		}
		...
		if(isset($_REQUEST["reports"]) && count($certificatesArray) != count(explode(",", $_REQUEST["reports"])))
		{
			...
			$diamondsCollection->addFieldToFilter('certificate', $likeReports);
		}
		*/
		...
		if($_REQUEST["order_by"])
		{
			...
			if($_REQUEST["order_by"] == "price")
				...
				$orderby = "certificate";
			else if($_REQUEST["order_by"] == "inhouse")
				$orderby = "INHOUSE";
			else if($_REQUEST["order_by"] == "l:w")
				$orderby = "dimensions";
		...
		if($orderby != "INHOUSE") $diamondsCollection->setOrder($orderby, $orderMethod);
		...
		if($_REQUEST["is_mobile"]){
			...
			if($orderby == "INHOUSE")
				$this->sort_array_of_array($diamonds, 'InHouse', strtolower($orderMethod));
			//print_r($diamonds);
    ...
    public function sortrcAction()
    {
		...
		if($post["table_name"] == "search_recently_header_table")
		{
			...
			$diamonds = Mage::helper("diamondsearch")->getDiamondsArray($stones, $_REQUEST["is_fancy"]);
			if($orderby == "INHOUSE")
				$this->sort_array_of_array($diamonds, 'InHouse', strtolower($post["order_method"]));
