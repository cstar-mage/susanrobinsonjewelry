...
class Ideal_Cronjobs_Model_Observer {
	...
	const XML_PATH_IDEALINFO_ENABLED = 'cronjobs/production/idealinfo_enabled';
	const XML_PATH_SYSHEALTH_ENABLED = 'cronjobs/production/syshealth_enabled';
    ...
    public function idealinfoCron() {
    	if (!Mage::getStoreConfigFlag(self::XML_PATH_IDEALINFO_ENABLED)) {
    		return;
    	}
		$response = Mage::helper("cronjobs")->idealinfoCron();
		
		if($response)echo "Ideal Info run Successful.";
		else echo "Ideal Info run Error.";
    	return $this;
    }
	
    public function syshealthCron() {
    	if (!Mage::getStoreConfigFlag(self::XML_PATH_SYSHEALTH_ENABLED)) {
    		return;
    	}
		$response = Mage::helper("cronjobs")->syshealthCron();
		
		if($response["status"])echo $response["msg"];
