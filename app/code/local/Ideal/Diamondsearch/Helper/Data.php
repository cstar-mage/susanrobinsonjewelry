...
class Ideal_Diamondsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
	function getDiamondsArray($stones, $is_fancy, $is_specialshape)
	{
		...
		//diamonds_owner_mask
		$owner_mask_data = $this->getOwnerMaskArray();
		
		foreach ( $stones as $stone) {
			
			$color = $stone['color'];
			if($is_fancy) $color = $this->getFullFancyIntensity($stone['fancy_intensity'])." ".$stone['fancycolor'];
			...
			*/
			$image_or_video_link = trim($stone['image_or_video_link']);
			if($image_or_video_link && !preg_match('/^<iframe /',$image_or_video_link))
				$image_or_video_link = '<iframe width="100%" height="100%" frameborder="0" style="min-height:500px;" src="'.trim($stone['image_or_video_link']).'" seamless="seamless"></iframe>';
			...
			//Owner Mask
			$owner = strtoupper($stone['owner']);
			if(!empty($owner_mask_data))
				if(array_key_exists($stone['owner'], $owner_mask_data))
					$owner = strtoupper($owner_mask_data[$stone['owner']]);
			...
			$diamonds[] = array(
				...
				"lab_comments" => $stone['lab_comments'],
				"dealer_comments" => $stone['dealer_comments'],
				//"gem_advisor" => $stone['gem_advisor'],
				//"helium_report" => $stone['make'],
				//"image" => $stone['diamond_image'],
				"image" => $diamond_image,
				"is_samp" => $is_samp,
				"video" => $image_or_video_link ? $image_or_video_link : false,
	...
	function getOwnerMaskArray(){
		$owner_mask = array();
		if( Mage::getStoreConfig("diamondsearch/general_settings/diamonds_owner_mask") ){
			$owner_mask_value = trim(Mage::getStoreConfig("diamondsearch/general_settings/diamonds_owner_mask"));
			$owner_mask_array = explode(",", $owner_mask_value);
			$owner_mask_array = array_map("trim",array_filter($owner_mask_array));
			foreach($owner_mask_array as $owner=>$mask){
				$mask_arr = explode("=", $mask);
				$mask_arr = array_map("trim",array_filter($mask_arr));
				$owner_mask[trim($mask_arr[0])] = trim($mask_arr[1]);
			}
		}
		return $owner_mask;
	}
	...
	function getInitSearchPanelDataJson()
	{
		...
		*/
		...
		if($diamondsCollection->getSize()){
			...
			$respArray["min_totalprice"] = round(floor($mainColl[0]["min_totalprice"]))-1;
			$respArray["max_totalprice"] = round(ceil($mainColl[0]["max_totalprice"]))+1;
	...
	function getShapeRangesJson()
	{
		...
		{
			$min_totalprice[] = $diam["min_totalprice"]-1;
			$max_totalprice[] = $diam["max_totalprice"]+1;
			$min_carat[] = $diam["min_carat"];
			$max_carat[] = $diam["max_carat"];
			
			$respArray[ucfirst(strtolower($diam["shape"]))] = array(
				(float)$diam["min_totalprice"]-1, 
				(float)$diam["max_totalprice"]+1,
	...
	function getFullFancyIntensity($value)
	{ 
