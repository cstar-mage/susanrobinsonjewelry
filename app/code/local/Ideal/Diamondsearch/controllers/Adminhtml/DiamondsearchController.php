...
class Ideal_Diamondsearch_Adminhtml_DiamondsearchController extends Mage_Adminhtml_Controller_action
{
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				$config -> saveConfig('diamondsearch/general_settings/diamond_shape_title', $data["diamond_shape_title"] , 'default', $storeId);				
				$config -> saveConfig('diamondsearch/general_settings/diamond_found_title', $data["diamond_found_title"] , 'default', $storeId);
				...
				$config -> saveConfig('diamondsearch/general_settings/diamondsview_shape_title', $data["diamondsview_shape_title"] , 'default', $storeId);
				$config -> saveConfig('diamondsearch/general_settings/diamondsview_carat_title', $data["diamondsview_carat_title"] , 'default', $storeId);
				$config -> saveConfig('diamondsearch/general_settings/diamonds_owner_mask', $data["diamonds_owner_mask"] , 'default', $storeId);
				...
				/*Font Setting save*/
				//$config -> saveConfig('diamondsearch/general_settings/diamond_default_fonts', $data["diamond_default_fonts"], 'default', $storeId);
				$config -> saveConfig('diamondsearch/general_settings/diamond_fonts_enable_googlefont', $data["diamond_fonts_enable_googlefont"], 'default', $storeId);
				$config -> saveConfig('diamondsearch/general_settings/diamond_fonts_all', $data["diamond_fonts_all"], 'default', $storeId);
	...
	public function resetCertificatesAction()
    {
		/*
		//CURRENT DATA
		$Attribute = array(
			array("label"=>"GIA","DetailsCode"=>"GIA","sortorder"=>"10","available"=>"1"),
			array("label"=>"EGL","DetailsCode"=>"EGL","sortorder"=>"20","available"=>"1"),
			array("label"=>"EGL International","DetailsCode"=>"EGL International","sortorder"=>"30","available"=>"1"),
			array("label"=>"EGL USA","DetailsCode"=>"EGL USA","sortorder"=>"40","available"=>"1"),
			array("label"=>"EGL ISR","DetailsCode"=>"EGL ISR","sortorder"=>"50","available"=>"1"),
			array("label"=>"GCAL","DetailsCode"=>"GCAL","sortorder"=>"60","available"=>"1") ,
			array("label"=>"AGS","DetailsCode"=>"AGS","sortorder"=>"70","available"=>"1"),	
			array("label"=>"HRD","DetailsCode"=>"HRD","sortorder"=>"80","available"=>"1"),
			array("label"=>"IGI","DetailsCode"=>"IGI","sortorder"=>"90","available"=>"1"),
			array("label"=>"AGL","DetailsCode"=>"AGL","sortorder"=>"100","available"=>"1"),
			array("label"=>"AGI","DetailsCode"=>"AGI","sortorder"=>"110","available"=>"1"),
			array("label"=>"NONE","DetailsCode"=>"NONE","sortorder"=>"120","available"=>"1"),
			array("label"=>"Non-Certified","DetailsCode"=>"Non-Certified","sortorder"=>"130","available"=>"1")
			);
		echo serialize($Attribute); exit;
		*/
		$tableName = Mage::getSingleton('core/resource')->getTableName('core_config_data');
		$where = "`path` = 'diamondsearch/slider_settings/certificate_slider'";
		$write = Mage::getSingleton('core/resource')->getConnection('core_write');
		$write->query("delete from ".$tableName." where ".$where);
		
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('diamondsearch')->__('Certificates reset successfully'));
		$this->_redirect('*/*/settings');
		return;
