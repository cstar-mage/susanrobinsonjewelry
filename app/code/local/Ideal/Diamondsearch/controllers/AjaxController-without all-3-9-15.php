...
class Ideal_Diamondsearch_AjaxController extends Mage_Core_Controller_Front_Action
{
    ...
    public function listAction()
    {
		...
		{
			$diamondsCollection->addFieldToFilter('fancycolor', array('in' => Mage::helper("diamondsearch")->getShortCodes("fancycolor", explode(",", $_REQUEST["fancycolors"])) ));
		}
		if(isset($_REQUEST["clarities"]) && count($claritiesArray) != count(explode(",", $_REQUEST["clarities"]))) 
		{
			$diamondsCollection->addFieldToFilter('clarity', array('in' => explode(",", $_REQUEST["clarities"])));
		}
		...
		{
			$diamondsCollection->addFieldToFilter('lotno', array('like' => "%".$_REQUEST["shipping_day"]."%"));
		}
		...
		/*$selQuery = $diamondsCollection->getSelect()." CNT: ".count($stones);
		echo $diamondsCollection->getSelect();
